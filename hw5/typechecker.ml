...
let rec typecheck_exp (c : Tctxt.t) (e : Ast.exp node) : Ast.ty =
  ...
  | CArr (arrty, exps) -> (
      if List.for_all (fun x -> subtype c (typecheck_exp c x) arrty) exps then (
        ...
        TRef (RArray arrty)
      ) else type_error e ("Array element expressions either not well-typed or not subtypes")
    )
  (* | NewArr (arrty, lenexp, index, initexp) -> (
      (* CONTINUE HERE (Generated by Copilot) *)
      let lenexp_ty = typecheck_exp c lenexp in
      let index_ty = typecheck_exp c index in
      let initexp_ty = typecheck_exp c initexp in
      match lenexp_ty, index_ty, initexp_ty with
        | TInt, TInt, _ -> (
          match arrty with
            | TRef (RArray arrty') -> (
              if subtype initexp_ty arrty' then (
                typecheck_ty e c arrty;
                NewArr (arrty, lenexp, index, initexp)
              ) else type_error e ("Array element expression not subtype of array type")
            )
            | _ -> type_error e ("Array type not a reference type")
        )
        | _ -> type_error e ("Array length or index expression not of type int")
    ) *)
